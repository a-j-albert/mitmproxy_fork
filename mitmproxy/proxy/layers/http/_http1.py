...
class Http1Server(Http1Connection):
    ...
    def send(self, event: HttpEvent) -> layer.CommandGenerator[None]:
        ...
        if isinstance(event, ResponseHeaders):
            ...
            assert self.response
            transfer_encoding = "" if not any("transfer-encoding" == item for item in self.response.headers) else self.response.headers["transfer-encoding"]
...
class Http1Client(Http1Connection):
    ...
    def send(self, event: HttpEvent) -> layer.CommandGenerator[None]:
        ...
        if isinstance(event, RequestHeaders):
            ...
            assert self.request
            # if "chunked" in self.request.headers.get("transfer-encoding", "").lower():
            transfer_encoding = "" if not any("transfer-encoding" == item for item in self.request.headers) else self.request.headers["transfer-encoding"]
            if "chunked" in transfer_encoding.lower():
                yield commands.SendData(self.conn, b"0\r\n\r\n")
